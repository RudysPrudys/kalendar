export default async function handler(req, res) {
  ...
  // OPRAVENO: Parametrizace Open-Meteo na správné weather_code podle standardu 2026
  ...
  try {
    ...
    try {
      const wResponse = await fetch(weatherUrl, { headers: { 'User-Agent': 'Elecrow-Panel' } });
      if (wResponse.ok) weatherData = await wResponse.json();
    } catch (e) {
      console.error("Meteo failed");
    }
    ...
    // Zamčeno na 4 události, aby dole zbylo místo na meteo lištu
      ...
      budouciEvents.forEach(ev => {
        ...
        htmlEvents += `
          <div style="display:flex; align-items:center; background:#2c2c2e; padding:12px 15px; margin-bottom:10px; border-radius:12px; border-left:4px solid #0a84ff;">
            ...
            <div style="flex:1;">
              <div style="font-size:19px; font-weight:600; color:#f5f5f7; margin-bottom:4px;">${cleanSummary}</div>
    ...
    // 4. OPRAVENÉ GENEROVÁNÍ BLOKU POČASÍ (Změněno na daily.weather_code)
    let htmlWeather = "";
    if (weatherData && weatherData.daily && weatherData.daily.weather_code) {
      const interpretWmoCode = (code) => {
        const c = Number(code);
        if (c === 0) return { txt: "Jasno", ico: "☀️" };
        if (c <= 3) return { txt: "Polojasno", ico: "⛅" }; 
        if (c <= 48) return { txt: "Mlhavo", ico: "🌫️" };
        if (c <= 55) return { txt: "Mrholení", ico: "🌧️" };
        if (c <= 65) return { txt: "Déšť", ico: "🌧️" };
        if (c <= 77) return { txt: "Sněžení", ico: "❄️" };
