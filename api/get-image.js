export default async function handler(req, res) {
  ...
  // 1. Paralelní načtení kalendáře i počasí přímo na serveru
  try {
  ...
  // 3. Formátování českého data na levém panelu
  ...
  // 4. Sestavení HTML pro kalendář
  let htmlEvents = "";
  if (budouciEvents.length === 0) {
    htmlEvents = `<div style="color:#86868b; text-align:center; padding:50px 0; font-size:16px; font-weight:500;">Žádné nadcházející události</div>`;
  } else {
    budouciEvents.forEach(ev => {
      ...
      htmlEvents += `
        <div style="display:flex; align-items:center; background:#2c2c2e; padding:10px 14px; margin-bottom:6px; border-radius:10px; border-left:4px solid #0a84ff;">
          <div style="background:#1c1c1e; padding:6px 10px; border-radius:6px; text-align:center; min-width:48px; margin-right:14px;">
          ...
          <div style="flex:1; min-width:0;">
            <div style="font-size:16px; font-weight:600; color:#f5f5f7; margin-bottom:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${cleanSummary}</div>
  ...
  // 5. Sestavení HTML pro počasí (přímo na serveru)
  let htmlWeather = "";
    ...
    for (let i = 0; i < 3; i++) {
      ...
      htmlWeather += `
        <div class="weather-day">
          <span style="font-weight:700; color:#a0a0ab; text-transform:uppercase; font-size:11px; margin-bottom:4px;">${denLabel}</span>
          <span style="font-size:26px; margin:4px 0;">${wInfo.ico}</span>
          <span style="font-weight:600; font-size:15px;">${maxT}° / <span style="color:#71717a;">${minT}°</span></span>
        </div>`;
    }
  } else {
    htmlWeather = `<div style="color:#ef4444; font-size:13px; width:100%; text-align:center; padding:10px 0;">Chyba načítání počasí</div>`;
  }

  // 6. Výsledný statický HTML kód
  const finalHtml = `
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <style>
        * { box-sizing: border-box; }
        ...
        .weather-box { display:flex; justify-content:space-between; background:#18181b; border:1px solid #27272a; padding:16px; border-radius:12px; width:100%; }
        .weather-day { text-align: center; flex: 1; color: #f5f5f7; font-size: 14px; }
    ...
    <body>
      <div class="dashboard">
        <div class="left-panel">
          ...
          <div style="font-size: 24px; font-weight: 600; color: #a0a0ab; margin-bottom: 30px;">${jmenoMesice}</div>
